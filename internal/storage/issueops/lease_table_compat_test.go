...
//
// NOT covered here: issueops.GetIssueInTx. be-cm3 degraded that path too, but
// it is the single-row hydration behind `bd show`, so a stripped overlay there
// answers "no live lease" for a database whose leases table is gone and leaves
// no way to tell that from a healthy unleased row. It refuses instead; the pin
// for that lives beside it in get_issue_test.go
// (TestGetIssueInTxMissingLeasesTableIsAnError); see be-bz4.

// TestGetIssueInTxUnrelatedErrorPropagates is the control: this path keeps the
// sqlbuild.LeaseJoin in its FROM clause and does not retry around it (be-bz4),
// so nothing here may swallow a non-ErrNotFound failure.
